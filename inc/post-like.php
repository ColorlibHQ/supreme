...
add_action( 'wp_enqueue_scripts', 'supreme_sl_enqueue_scripts' );
function supreme_sl_enqueue_scripts() {

	wp_enqueue_script( 'supreme-post-likes', SUPREME_DIR_JS_URI . 'post-likes.js', array( 'supreme-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_supreme_process_simple_like', 'supreme_process_simple_like' );
add_action( 'wp_ajax_supreme_process_simple_like', 'supreme_process_simple_like' );
function supreme_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'supreme' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'supreme' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'supreme' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'supreme' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'supreme' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'supreme' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'supreme_show_user_likes' );
add_action( 'edit_user_profile', 'supreme_show_user_likes' );
function supreme_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'supreme' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'supreme' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
